...
class Edit_My_Account extends StatefulWidget {
    const Edit_My_Account({Key? key}) : super(key: key);

    @override
    _Edit_My_AccountState createState() => _Edit_My_AccountState();
}

class _Edit_My_AccountState extends State<Edit_My_Account> {
        ...
        try {
            ...
            showDialog(
                ...
                builder: (BuildContext context) {
                    return AlertDialog(
                        title: Text("Notification"),
                        content: Text("An error occurred while updating the account"),
                        actions: <Widget>[
                            TextButton(
                                child: Text("Close"),
                                onPressed: () {
    ...
    DateTime calculateBirthDate(int aged) {
        final currentDate = DateTime.now();
        final birthYear = currentDate.year - aged;
        final birthDate = DateTime(birthYear, currentDate.month, currentDate.day);
        return birthDate;
    }
    ...
    void getUserInfo() async {
        ...
        try {
            ...
            if (response.statusCode == 200) {
                ...
                final filteredUser = fetchedUsers.cast<Map<String, dynamic>>().firstWhere(
                        (user) => user['ID'] == userId,
                    orElse: () => {},
                );
                ...
                setState(() {
                    ...
                    int age = int.parse(filteredUser['Age']);
                    final currentDate = DateTime.now();
            ...
            showDialog(
                ...
                builder: (BuildContext context) {
                    return AlertDialog(
                        title: Text("Notification"),
                        content: Text("An error occurred while fetching user information"),
                        actions: <Widget>[
                            TextButton(
                                child: Text("Close"),
                                onPressed: () {
